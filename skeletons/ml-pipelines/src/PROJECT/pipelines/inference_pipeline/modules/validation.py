...
The trainer already realigns columns internally; this module exists so
the *pipeline* can fail with a message about the input file before a
model is asked to predict on it, and so the checks are testable without
...
def validate_features(df: pd.DataFrame, feature_columns: list[str]) -> pd.DataFrame:
    ...
    Missing features raise - a silently absent column becomes an all-NaN
    imputed column and a plausible wrong answer. Extra columns only warn,
    because a model-input table legitimately carries keys and labels.
...
def validate_keys(df: pd.DataFrame, key_cols: list[str]) -> list[str]:
    ...
    Missing keys are a warning, not an error: a serving payload may
    legitimately have none. But the evaluation pipeline joins on them, so
